...
def calculate_shadow_direction(light_vector, angle_offset=0):
    light_vector = np.array(light_vector, dtype=float)
    ...
    z_component = light_vector[2]

    new_vector = np.array([x_new, y_new, z_component])
    norm = np.linalg.norm(new_vector)
    new_light_vector = new_vector / norm

    new_light_vector *= -1.0 / new_light_vector[2]

    return new_light_vector, theta
