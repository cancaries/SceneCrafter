...
class CameraSimple(nn.Module):
    def __init__(
        ...
        self.ego_pose = torch.from_numpy(ego_pose).float().cuda()
            
        self.extrinsic = torch.from_numpy(extrinsic).float().cuda()
...
def loadCam_custom(cam_info: CameraInfo):
    orig_w, orig_h = cam_info.image.size
    if cfg.resolution == -1:
        if orig_w > 1600:
            global WARNED
...
def cameraList_from_camInfos(cam_infos, resolution_scale, render_flag=False):
    camera_list = []
