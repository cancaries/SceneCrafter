...
from lib.utils.waymo_utils import is_within_distance
class StreetGaussianModel(nn.Module):
    def __init__(self, metadata):
        ...
        self.light_direction = metadata['light_direction']
        self.setup_functions() 
    
    def set_visibility(self, include_list):
        self.include_list = include_list # prefix

    def set_obj_visibility(self, obj_visibility):
        self.include_obj = obj_visibility
        ...
        if self.include_obj:
            for track_id, obj_meta in self.obj_info.items():
                ...
                if track_id in self.agents_id:
                    ...
                    model.set_global_light_direction(self.light_direction)
    ...
    def parse_camera(self, camera: Camera):
        ...
        if self.include_obj:
            ...
            for i, obj_name in enumerate(self.obj_list):
                ...
                if track_id in self.frame_vehicle_data[frame_idx].keys():
        ...
        if len(self.graph_obj_list) > 0:
            ...
            for i, obj_name in enumerate(self.graph_obj_list):
                ...
                if track_id not in self.metadata['agents_id']:
                    ...
                    obj_yaw = self.frame_vehicle_data[self.viewpoint_camera.render_frame_idx][track_id]['rot']
                    obj_model.set_cur_yaw(obj_yaw)
