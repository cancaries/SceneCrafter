...
class StreetGaussianVisualizer():
    ...
    def visualize(self, result, camera: Camera, vis_depth=True):
        ...
        if self.save_image:
            # resize to 1600x900
            # rgb = torch.nn.functional.interpolate(rgb.unsqueeze(0), size=(900, 1600), mode='bilinear', align_corners=False).squeeze(0)
            # rgb_bkgd = torch.nn.functional.interpolate(rgb_bkgd.unsqueeze(0), size=(900, 1600), mode='bilinear', align_corners=False).squeeze(0)
            # rgb_bkgd_wo_sky = torch.nn.functional.interpolate(rgb_bkgd_wo_sky.unsqueeze(0), size=(900, 1600), mode='bilinear', align_corners=False).squeeze(0)
            # rgb_obj = torch.nn.functional.interpolate(rgb_obj.unsqueeze(0), size=(900, 1600), mode='bilinear', align_corners=False).squeeze(0)
            # acc_obj = torch.nn.functional.interpolate(acc_obj.unsqueeze(0), size=(900, 1600), mode='bilinear', align_corners=False).squeeze(0)
