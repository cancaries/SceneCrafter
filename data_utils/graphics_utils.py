...
from cal_utils import bbox_to_corner3d
...
def project_numpy(xyz, K, RT, H, W):
    ...
    Args:
        xyz (np.ndarray): 3D point cloud with shape [N, 3]
        K (np.ndarray): Camera intrinsic matrix with shape [3, 3]
        RT (np.ndarray): Camera extrinsic matrix (world to camera) with shape [4, 4]
        H (int): Image height in pixels
        W (int): Image width in pixels
        
    Returns:
        tuple: (xyz_pixel, mask) where
            - xyz_pixel: 2D coordinates on image plane [N, 2]
            - mask: Boolean mask indicating valid points in camera frustum [N]
    """
...
def project_label_to_image(dim, obj_pose, calibration, img_width=1920, img_height=1280):
    """
    Project 3D object bounding box to 2D image coordinates.
    
    This function takes 3D object dimensions and pose, then projects the bounding box
    corners to 2D image coordinates using camera calibration parameters.
    
    Args:
        dim (tuple): Object dimensions (width, length, height) in meters
        obj_pose (np.ndarray): 4x4 object pose matrix in world coordinates
        calibration (dict): Camera calibration containing 'extrinsic' and 'intrinsic' keys
        img_width (int): Image width in pixels
        img_height (int): Image height in pixels
        
    Returns:
        tuple: (points_uv, valid) where
            - points_uv: 2D image coordinates [8, 2] for bounding box corners
            - valid: Boolean array [8] indicating valid projections
    """
